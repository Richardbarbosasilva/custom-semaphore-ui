...
func tryFindLDAPUser(username, password string) (*db.User, error) {
	ldapUser, err := findLDAPUserWithConfig(getCurrentLDAPRuntimeConfig(), username, password)
	if err != nil {
		return nil, err
	}
	...
	log.Info("LDAP authorized user: " + string(jsonBytes))
	return ldapUser, nil
}
...
func login(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		config := &loginMetadata{
			...
			LdapEnabled:       util.Config.LdapEnable,
		}
		i := 0
		...
		if util.GetTotpConfig().Enabled {
			config.AuthMethods.Totp = &LoginTotpAuthMethod{
				AllowRecovery: util.GetTotpConfig().AllowRecovery,
			}
		}
