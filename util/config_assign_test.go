...
func TestAssignMapToStruct_PointerNestedStructs(t *testing.T) {
	type Inner struct {
		Enabled bool   `json:"enabled"`
		Issuer  string `json:"app_name"`
	}

	type Outer struct {
		Inner *Inner `json:"totp,omitempty"`
	}

	t.Run("initializes nil pointer and assigns nested values", func(t *testing.T) {
		var o Outer

		if err := AssignMapToStruct(map[string]any{
			"totp": map[string]any{
				"enabled":  "true",
				"app_name": "I-Mais Semaphore",
			},
		}, &o); err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		...
		if o.Inner == nil {
			t.Fatal("expected nested pointer to be initialized")
		}

		if !o.Inner.Enabled || o.Inner.Issuer != "I-Mais Semaphore" {
			t.Fatalf("unexpected nested pointer content: %+v", o.Inner)
		}
	})

	t.Run("merges into existing pointer without clearing other fields", func(t *testing.T) {
		o := Outer{
			Inner: &Inner{
				Enabled: false,
		...
		if o.Inner == nil {
			t.Fatal("expected nested pointer to remain initialized")
		}
		...
		if o.Inner.Issuer != "Existing Issuer" {
			t.Fatalf("expected Issuer to be preserved, got %q", o.Inner.Issuer)
		}
	})
}

func TestAssignMapToStruct_ConfigAuthTotp(t *testing.T) {
	cfg := ConfigType{}
	...
	if err := AssignMapToStruct(map[string]any{
		...
		t.Fatalf("unexpected error assigning auth.totp config: %v", err)
	}

	if cfg.Auth == nil || cfg.Auth.Totp == nil {
		t.Fatal("expected auth.totp pointer chain to be initialized")
	}

	if !cfg.Auth.Totp.Enabled || !cfg.Auth.Totp.AllowRecovery || cfg.Auth.Totp.Issuer != "I-Mais Semaphore" {
		t.Fatalf("unexpected auth.totp config: %+v", cfg.Auth.Totp)
	}
}
