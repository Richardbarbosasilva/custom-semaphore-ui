    ...
    >
      <div class="app-sidebar">
        <div class="app-sidebar__brand">
          <div class="app-sidebar__brand-card">
            <img
              :src="brandLogo"
              alt="iMais"
          ...
          <div class="app-sidebar__brand-caption">{{ $t('sidebar_brand_caption') }}</div>
        </div>
        ...
        <v-menu bottom max-width="235" max-height="100%" v-if="project">
          <template v-slot:activator="{ on, attrs }">
            <v-card
              flat
              class="app-sidebar__project-card"
              v-bind="attrs"
              v-on="on"
              data-testid="sidebar-currentProject"
            >
              <div class="app-sidebar__project-row">
                <v-avatar
                  :color="getProjectColor(project)"
                  size="38"
                  class="app-sidebar__project-avatar"
                >
                  <span class="white--text">{{ getProjectInitials(project) }}</span>
                </v-avatar>

                <div class="app-sidebar__project-copy">
                  <div class="app-sidebar__project-name">{{ project.name }}</div>
                  <div class="app-sidebar__project-role">{{ currentProjectRoleLabel }}</div>
                </div>

                <div class="app-sidebar__project-arrow">
          ...
          <v-list dense class="app-sidebar__menu-list">
            <v-list-item
        ...
        <div v-if="project" class="app-sidebar__sections">
          <div
            v-for="section in projectNavigationSections"
            :key="section.key"
            class="app-sidebar__section"
          >
            <div class="app-sidebar__section-title">{{ section.title }}</div>

            <v-list dense nav class="app-sidebar__nav-list">
              <v-list-item
              ...
              >
                <v-list-item-icon class="app-sidebar__nav-item-icon-wrap">
                  <div class="app-sidebar__nav-item-icon">
        ...
        <v-list v-else dense nav class="app-sidebar__nav-list app-sidebar__nav-list--standalone">
          <v-list-item
            key="new_project"
            :to="`/project/new`"
            class="app-sidebar__nav-item"
          ...
          >
            <v-list-item-icon class="app-sidebar__nav-item-icon-wrap">
              <div class="app-sidebar__nav-item-icon">
                <v-icon small>mdi-shield-key-outline</v-icon>
              </div>
            </v-list-item-icon>

            <v-list-item-content>
              <v-list-item-title>Autenticação</v-list-item-title>
            </v-list-item-content>
          </v-list-item>
        </v-list>
      </div>
      ...
      <template v-slot:append>
        <div class="app-sidebar__footer">
          <div class="app-sidebar__utility-card">
            <v-btn
              icon
              class="app-sidebar__utility-btn"
            ...
            <v-menu top min-width="150" max-width="235" nudge-top="12" :position-x="50" absolute>
              <template v-slot:activator="{ on, attrs }">
                <v-btn icon class="app-sidebar__utility-btn" v-bind="attrs" v-on="on">
                  <img
                    class="app-sidebar__flag"
            ...
            <div class="app-sidebar__version" v-if="systemInfo">
              {{ systemInfo.version }}
            </div>
          </div>
          ...
          <v-menu top max-width="235" nudge-top="12">
            <template v-slot:activator="{ on, attrs }">
              <v-card
                flat
                class="app-sidebar__account-card"
                v-bind="attrs"
                v-on="on"
              >
                <div class="app-sidebar__account-row">
                  <div class="app-sidebar__account-avatar">
                    <span>{{ userInitials }}</span>
                  </div>

                  <div class="app-sidebar__account-copy">
                    <div class="app-sidebar__account-name">{{ user.name }}</div>
                    <div class="app-sidebar__account-role">{{ accountRoleLabel }}</div>
                  </div>

                  <div class="app-sidebar__account-pill" v-if="user.admin">
                    {{ $i18n.t('admin') }}
                  </div>
                </div>
              </v-card>
            ...
            <v-list>
              ...
              <v-list-item key="authentication" to="/authentication" v-if="user.admin">
                <v-list-item-icon>
...
.NavDrawer {
  ...
  .v-navigation-drawer__content {
    background:
      radial-gradient(circle at top, rgba(86, 152, 255, 0.26), transparent 24%),
      linear-gradient(180deg, #17172b 0%, #131727 48%, #0d1321 100%);
  }
}
...
.app-sidebar__brand-caption,
.app-sidebar__section-title,
.app-sidebar__version,
.app-sidebar__account-role,
.app-sidebar__project-role {
...
.app-sidebar__project-card,
.app-sidebar__utility-card,
.app-sidebar__account-card {
  background: rgba(255, 255, 255, 0.08) !important;
  border: 1px solid rgba(255, 255, 255, 0.08);
  box-shadow: 0 14px 32px rgba(6, 10, 20, 0.22);
}
...
.app-sidebar__project-card:hover,
.app-sidebar__account-card:hover {
  box-shadow: 0 18px 36px rgba(6, 10, 20, 0.28);
...
.app-sidebar__project-avatar {
  font-size: 14px;
  font-weight: 700;
}

.app-sidebar__project-copy,
...
.app-sidebar__project-role,
.app-sidebar__account-role {
  color: rgba(217, 223, 255, 0.66);
  font-size: 10px;
  font-weight: 700;
  margin-top: 4px;
}

.app-sidebar__project-arrow,
.app-sidebar__account-pill {
  flex-shrink: 0;
}
...
.app-sidebar__nav-list--standalone {
  margin-top: 4px;
}

.app-sidebar__nav-item {
  border-radius: 18px !important;
  color: rgba(239, 243, 255, 0.88) !important;
  margin-bottom: 6px;
...
.app-sidebar__nav-item.v-list-item--active,
.app-sidebar__nav-item--active {
  background: linear-gradient(90deg, rgba(56, 131, 255, 0.96), rgba(0, 201, 255, 0.72)) !important;
  box-shadow: 0 14px 28px rgba(32, 86, 210, 0.28);
  color: #ffffff !important;
}

.app-sidebar__nav-item .v-list-item__icon {
  align-self: center;
  margin: 0 12px 0 0 !important;
}

.app-sidebar__nav-item .v-list-item__title {
  font-size: 14px !important;
  font-weight: 600;
}

.app-sidebar__nav-item-icon {
  align-items: center;
...
.app-sidebar__nav-item.v-list-item--active .app-sidebar__nav-item-icon,
.app-sidebar__nav-item--active .app-sidebar__nav-item-icon {
...
.app-sidebar__utility-btn {
  background: rgba(255, 255, 255, 0.08) !important;
}

.app-sidebar__utility-btn .v-icon {
...
import brandLogo from '@/assets/i_mais_white.png';
...
export default {
  ...
  computed: {
    ...
    currentProjectRoleLabel() {
      return (this.userRole || {}).role || this.$t('users');
    },
    ...
    accountRoleLabel() {
      ...
      return this.currentProjectRoleLabel;
    },
    ...
    projectNavigationSections() {
      ...
      const workspace = [{
        ...
        to: `/project/${this.projectId}/overview`,
      ...
      if (this.project.type === '') {
        ...
        workspace.push({
          ...
          to: `/project/${this.projectId}/schedule`,
          testId: 'sidebar-schedule',
        });
      }

      const resources = this.project.type === '' ? [{
        key: 'inventory',
        title: this.$t('inventory'),
        icon: 'mdi-monitor-multiple',
        to: `/project/${this.projectId}/inventory`,
        testId: 'sidebar-inventory',
      }, {
        key: 'environment',
        title: this.$t('environment'),
        icon: 'mdi-code-braces',
        to: `/project/${this.projectId}/environment`,
        testId: 'sidebar-environment',
      }, {
        key: 'keys',
        title: this.$t('keyStore'),
        icon: 'mdi-key-change',
        to: `/project/${this.projectId}/keys`,
        testId: 'sidebar-keys',
      }, {
        key: 'repositories',
        title: this.$t('repositories'),
        icon: 'mdi-git',
        to: `/project/${this.projectId}/repositories`,
      }, {
        key: 'integrations',
        title: this.$t('integrations'),
        icon: 'mdi-connection',
        to: `/project/${this.projectId}/integrations`,
        testId: 'sidebar-integrations',
      }] : [];

      return [{
        key: 'workspace',
        title: this.$t('sidebar_workspace'),
        items: workspace,
      }, ...(resources.length > 0 ? [{
        key: 'resources',
        title: this.$t('sidebar_resources'),
        items: resources,
      }] : []), {
        key: 'access',
        title: this.$t('sidebar_access'),
        items: [{
          key: 'team',
          title: this.$t('team'),
          icon: 'mdi-account-group-outline',
          to: `/project/${this.projectId}/team`,
          testId: 'sidebar-team',
        }],
      }];
    },
          ...
          reader.onload = async (ev) => {
            ...
            try {
              await axios.post('/api/projects/restore', fileContent).then(async (payload) => {
                this.$router.push({ path: `/project/${payload.data.id}/overview` });
                this.state = 'success';
                await this.loadProjects();
              });
            } catch (err) {
